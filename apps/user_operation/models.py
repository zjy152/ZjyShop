...
class UserLeavingMessage(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, verbose_name='用户')
    message_type = models.IntegerField(default=1, choices=MESSAGE_CHOICES, help_text=u"留言类型: 1(留言),2(投诉),"
                                                                                     u"3(询问),4(售后),5(求购)",
                                       verbose_name='留言类型')
    subject = models.CharField(max_length=100, default='', verbose_name='主题')
    message = models.TextField(default='', help_text='留言内容', verbose_name='留言内容')
    file = models.FileField(upload_to='message/images/', help_text='上传的文件', verbose_name='上传的文件')
    add_time = models.DateTimeField(default=datetime.now, verbose_name='添加时间')
